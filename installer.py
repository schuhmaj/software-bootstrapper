...
def install_spec(
    config: dict[str, str], install_dir: Path, dry_run: bool = False
) -> Path:
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        logger.info(
            f"Configuring, building and installing {config['name']} {config['version']}"
        )
        install_dir.mkdir(parents=True, exist_ok=True)
        subprocess.run(
            ["cmake", "--workflow", "--preset", "default"], cwd=source_dir, check=True
        )
...
def install_module_file(
    ...
    module_file = (
        module_dir
        / config["name"]
        / f"{config['version']}{'.lua' if language == 'lua' else ''}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Filter specs to install. Must be a regex",
    )
    parser.add_argument(
        "-e",
        "--exclude",
        type=str,
        default=None,
        help="Filter specs to not install. Must be a regex",
    )
    parser.add_argument(
        "-l",
        "--language",
    ...
    for config in spec_configs:
        if (not include_regex or include_regex.match(config["name"])) and not (
            ...
            if args.dry_run:
                logger.info(
                    f"Would install {config['name']} {config['version']} to {args.install_dir}"
                )
            else:
                install_dir = install_spec(config, args.install_dir, args.skip_install)
                if args.module_dir is not None:
                    install_module_file(
                        config, install_dir, args.module_dir, args.language
                    )
